...
struct MMPsychoShift : public MM
{
    ...
    static void uas(int s, int t, BS &b) {
        ...
        if (b.poke(s).status() == Pokemon::Poisoned && b.poke(s).statusCount() > 0) {
...
struct MMRazorWind : public MM
{
    ...
    static void ms(int s, int, BS &b) {
        if (!poke(b,s).contains("ReleaseTurn") || poke(b,s)["ReleaseTurn"].toInt() != b.turn()) {
            ...
            int num = turn(b,s)["RazorWind_Arg"].toInt();
            b.sendMoveMessage(104, num == 7 ? 3 : num, s, type(b,s), s, move(b,s));
            /* Skull bash */
...
struct MMZExtremeEvoboost : public MM
{
    MMZExtremeEvoboost() {
        functions["UponAttackSuccessful"] = &uas;
    }
