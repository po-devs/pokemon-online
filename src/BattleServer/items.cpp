...
struct IMEscapeButton : public IM
{
    ...
    static void ubh(int s, int t, BS &b) {
        if (b.koed(s) || turn(b,t).value("EncourageBug").toBool() || (b.hasSubstitute(s) && !b.canBypassSub(t)) || turn(b,s)["SendingBack"] == true)
            return;
...
struct IMSafetyGoggles  : public IM {
    ...
    static void uodr(int s, int t, BS &b) {
        if (tmove(b,t).flags & Move::PowderFlag) {
            ...
            b.sendItemMessage(42, s, 0, 0, b.poke(s).item(), move(b,t));
        }
    }
};
