...
//////////////////////////////////
/* Global Hot Key.Currently, only work on Windows NT */
#if defined(WIN32) || defined(WIN64)
QGlobalHotKey::QGlobalHotKey(quint32 p_nativeMods, quint32 p_nativeKey, QString p_keyid)
{
    keyid=p_keyid;
    nativeMods=p_nativeMods;
    nativeKey=p_nativeKey;
    times=0;
    hide();
    setHotKey();
}

QGlobalHotKey::~QGlobalHotKey()
{
    clearHotKey();
}

bool QGlobalHotKey::setHotKey()
{
    WCHAR *hotkeyid=new WCHAR[keyid.size()];
    keyid.toWCharArray(hotkeyid);
    keyatom=GlobalAddAtomW(hotkeyid)-0xC000;
    delete [] hotkeyid;
    return RegisterHotKey(winId(),keyatom,nativeMods,nativeKey);
}

bool QGlobalHotKey::clearHotKey()
{
    return UnregisterHotKey(winId(), keyatom);
}

bool QGlobalHotKey::winEvent(MSG* msg, long* result)
{
