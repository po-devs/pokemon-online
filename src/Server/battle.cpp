...
bool BattleSituation::testStatus(int player)
{
    ...
    if (isConfused(player) && !(pokeMemory(player).contains("BlastBurnTurn") && pokeMemory(player)["BlastBurnTurn"].toInt() + 1 == turn())) {
...
int BattleSituation::calculateDamage(int p, int t)
{
    ...
    if (gen() != 2 || !crit || !turnMemory(p).value("CritIgnoresAll").toBool()) {
        damage = damage / (
                    ((poke.status() == Pokemon::Burnt || pokeMemory(p).value("WasBurnt").toBool()) && cat == Move::Physical && !hasWorkingAbility(p,Ability::Guts))
                    ? 2 : 1);
    }
