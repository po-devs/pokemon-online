...
struct MMSwitcheroo : public MM
{
    ...
    static void daf(int s, int t, BS &b) {
        if (b.koed(t) || (b.poke(t).item() == 0 && b.poke(s).item() == 0) || b.hasWorkingAbility(t, Ability::StickyHold)
                || (b.ability(t) == Ability::Multitype && (b.gen() <= 4 || ItemInfo::isPlate(b.poke(t).item()) || ItemInfo::isPlate(b.poke(s).item())))
                || (b.ability(s) == Ability::Multitype && (ItemInfo::isPlate(b.poke(t).item()) || ItemInfo::isPlate(b.poke(s).item())))
