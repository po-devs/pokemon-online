...
struct MMSecretPower : public MM {
    ...
    static void ms(int s, int, BS &b) {
        if (b.gen() == 5) {
            ...
            tmove(b,s).statAffected = Accuracy;
            tmove(b,s).rateOfStat = 30;
            tmove(b,s).boostOfStat = uchar(-1);
        } else {
...
struct MMOutrage : public MM
{
    ...
    static void uas(int s, int, BS &b) {
        ...
        if (b.gen() >= 5 || poke(b,s)["DisabledMove"] == Move::Outrage || poke(b,s)["DisabledMove"] == Move::Thrash || poke(b,s)["DisabledMove"] == Move::PetalDance)
