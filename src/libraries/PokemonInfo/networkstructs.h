...
struct UserInfo
{
    ...
    UserInfo (QString name = "", qint8 flags = Online, qint8 auth = 0, QString ip = "", QString date = "", QString os = "")
        : flags(flags), auth(auth), ip(ip), name(name), date(date), os(os)
    {
...
inline DataStream & operator << (DataStream &d, const UserInfo &ui) {
    d << ui.flags << ui.auth << ui.ip << ui.name << ui.date << ui.os;
    return d;
}
...
struct Battle
{
    ...
    quint8 mode;
    QString tier;
    Battle(int id1=0, int id2=0, int mode = 0, const QString &tier="");
};
