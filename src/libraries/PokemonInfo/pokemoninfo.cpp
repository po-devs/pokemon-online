...
QPixmap PokemonInfo::Sub(Pokemon::gen gen, bool back)
{
    ...
    if (gen <= 3) {
        ...
        file = QString("s-m/%1substitute.png").arg(back?"back/":"");
    }
...
QString MoveInfo::ZName(int movenum, bool zmove)
{
    ...
    if (zmove && movenum != 0 && movenum != Move::ExtremeEvoboost && (Power(movenum, Pokemon::gen()) == 0 || movenum == Move::MeFirst)) {
...
bool MoveInfo::canBeZMove(Pokemon::uniqueId pk, int item, int mv, Pokemon::gen gen)
{
    ...
    if (MoveInfo::isUniqueZMove(zmove)) {
        switch(zmove) {
            ...
            case Move::LightThatBurnsTheSky:
                return pk == Pokemon::Ultra_Necrozma && mv == Move::PhotonGeyser;
        }
    }
